...
interface DotenvPluginConfig extends Omit<DotenvPluginOptions, 'path' | 'prefix'> {
  path: string;
  prefix: string;
}
...
export class DotenvPlugin implements RspackPluginInstance {
  ...
  getDefaults() {
    ...
    if (defaults) {
      return this.loadFile(defaults === true ? `${path}.defaults` : defaults, silent);
    }
  ...
  formatData(variables: NodeJS.ProcessEnv, target: Target) {
    ...
    for (const [key, value] of Object.entries(variables)) {
      formatted[`${prefix}${key}`] = JSON.stringify(
        expand && typeof value === 'string' ? interpolate(value, variables) : value,
      );
    }
    ...
    return targets.every(
      (target) =>
        ...
        (this.config.ignoreStub === false ||
          ...
          (typeof target === 'string' && !target.includes('node') && !isMainThreadElectron(target))),
    );
  }
